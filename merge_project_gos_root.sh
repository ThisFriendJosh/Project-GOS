...
# Root mode: write into repo root (.)
SUBDIR="."
...
if [[ ! -d .git ]]; then echo "!! Not a git repo (run in repo root)"; exit 1; fi
  ...
  if [[ -f "$path" ]]; then
    if [[ $FORCE -eq 1 ]]; then
      if ! diff -q <(printf "%s" "$content") "$path" >/dev/null 2>&1; then
        cp "$path" "$path.bak.$TS"; printf "%s" "$content" > "$path"; echo "  ~ Updated (backup saved): $path"
      else
        echo "  = Unchanged: $path"
      fi
    else
    ...
    mkdir -p "$(dirname "$path")"; printf "%s" "$content" > "$path"; echo "  + Created: $path"
  fi
}

echo ">> Ensuring directory structure"
mkdir -p "${ROOT}"/core/{spice,disarm,mappers,metrics,engines} \
         "${ROOT}"/pipeline/{ingest,normalize,enrich,graph} \
         "${ROOT}"/sim "${ROOT}"/api "${ROOT}"/db "${ROOT}"/ui/webapp "${ROOT}"/ops

for d in core/spice core/disarm core/mappers core/metrics core/engines \
         pipeline/ingest pipeline/normalize pipeline/enrich pipeline/graph \
         ui/webapp
do
  f="${ROOT}/${d}/.gitkeep"; [[ -f "$f" ]] || { mkdir -p "$(dirname "$f")"; echo "" > "$f"; echo "  + Created: $f"; }
done

# --- files (root-safe imports/paths) ---
...
# requirements.txt at repo root (only if missing or --force)
if [[ -f "requirements.txt" && ${FORCE} -eq 0 ]]; then
  echo "  = Exists (kept): requirements.txt"
else
  write_file "requirements.txt" "$(cat <<'REQ'
fastapi==0.115.0
...
# Project-GOS Structure Drop (root mode)
...
# stage & commit
git add "${ROOT}" requirements.txt || true
if ! git diff --cached --quiet; then
  git commit -m "feat(Project-GOS): scaffold core/pipeline/sim/api/db/ui/ops structure (root-mode safe merge)"
else
  echo ">> Nothing to commit."
fi

# suggest push/PR
echo ">> Done. Push and open PR:"
